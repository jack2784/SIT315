...
void setup() {
  ...
  attachPinChangeInterrupt(pirTwo, PinChangeInterrupter, HIGH);
...
void loop(){
	...
	delay(5000); // simulate function running on loop to be interrupted
...
void toggleButton(){
  ...
  if (ButState == HIGH){
   digitalWrite(ledExternal, HIGH); // turn LED ON 
  }
  else{
   digitalWrite(ledExternal, LOW); // turn LED OFF
  } 
}


void PinChangeInterrupter() {
  val = digitalRead(11);  // read input value
  if (val == HIGH) {            // check if the input is HIGH
    if (pirStateTwo == LOW) {
      // we have just turned on
      Serial.println("Motion Sensor 2 HIGH");
      pirStateTwo = HIGH;
    }
  } else {
    if (pirStateTwo == HIGH){
      // we have just turned of
      Serial.println("Motion Sensor 2 LOW");
      pirStateTwo = LOW;
    }
  }
}
